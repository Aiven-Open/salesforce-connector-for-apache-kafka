...
/**
 * The BulkApiQueryEngine handles taking the config from the connector and making the relevant
 * queries to the Salesforce BulkApi 2.0 It handles the lifecycle of the requests along ith
 * exceptions
 */
public class BulkApiQueryEngine {
  ...
  /**
   * To be configured through config, the amount of time to wait in between executing the same query
   * again.
   */
   ...
   *
   * @param config the salesforceConfig
   * @param apiClient the BulkApiClient used for communication
   */
  public BulkApiQueryEngine(
  ...
  /**
   * GetRecords takes the preconfigured queries and executes those queries in order until no records
  ...
  public Iterator<BulkApiNativeInfo> getRecords(SOQLQuery query, String lastModifiedDate) {
    ...
    if (optJobId.isPresent()) {
      ...
      if (optQueryResponse.isPresent()) {
        ...
        LOGGER.debug(
            "JobId {} , State {}, Date Created {}, error message {}",
            queryResponse.getId(),
            queryResponse.getState(),
        ...
        switch (completedState) {
          ...
          default:
            LOGGER.warn(
                "State {} returned while waiting for query which was unexpected", completedState);
  ...
  class FutureIterator implements Iterator<BulkApiNativeInfo> {
    ...
    public boolean hasNext() {
      ...
      return bulkApiResultResponseFuture != null
          && !bulkApiResultResponseFuture.isCancelled()
    ...
    public BulkApiNativeInfo next() {
      if (hasNext()) {
        ...
        final NativeInfo<BulkApiKey, String> nativeInfo =
            bulkApiResultResponse.getResult().getNativeInfo();
        String topic =
            String.format(
                "%s.%s.%s",
                config.getTopicPrefix(),
                nativeInfo.nativeKey().getApiName(),
                bulkApiResultResponse.getResult().getObjectName());

        BulkApiNativeInfo bulkApiNativeInfo =
            new BulkApiNativeInfo(
                nativeInfo,
                topic,
                null,
                null,
        ...
        if (StringUtils.isNotBlank(bulkApiResultResponse.getLocator())
          ...
          bulkApiResultResponseFuture =
              apiClient.getJobResults(
                  jobId, bulkApiResultResponse.getLocator(), object, bulkApiKey);
          bulkApiKey =
              new BulkApiKey(
                  bulkApiKey.getApiName(),
                  bulkApiKey.getQueryHash(),
                  bulkApiKey.getLastExecutionTime(),
                  locator,
                  false);
        } else {
