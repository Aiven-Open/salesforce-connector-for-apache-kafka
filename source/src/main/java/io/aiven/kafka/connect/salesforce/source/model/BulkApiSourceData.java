...
/**
 * This BulkApiSourceData facilitates sending BulkApi data into a SourceRecord along with creating
 * the OffsetManager entry for it.
 */
public class BulkApiSourceData extends NativeSourceData<BulkApiKey> {
  ...
  private static final KeySerde<BulkApiKey> BULKAPIKEY_SERDE =
      new KeySerde<>() {
   ...
   *
   * @param config The SalesforceSourceConfigFragment with all the relevant config for configuring
   *     the BulkApiSourceData
   * @param offsetManager the offsetManager used in this implementation of BulkApiSourceData
   * @param lastSeenModifiedDate Map of last Seen LastModifiedDates processed
   */
   ...
   *
   * @param config The SalesforceSourceConfig
   * @param offsetManager The OffsetManager
   * @param engine A BulkApiQueryEngine
   * @param lastSeenModifiedDate last Seen LastModifiedDate processed
   */
  @VisibleForTesting
  protected BulkApiSourceData(
      final SalesforceSourceConfig config,
      final OffsetManager offsetManager,
  ...
  protected OffsetManager.OffsetManagerEntry createOffsetManagerEntry(final Context context) {
    ...
    return new SalesforceOffsetManagerEntry(
        (BulkApiKey) context.getNativeKey(),
        ctx.getJobId(),
        ctx.getLocator(),
        ctx.getTotalRecords(),
  ...
  /**
   * This method allows us to back off for a while and not constantly be trying to re run the same
   * query too quickly.
   */
  ...
  /**
   * getSalesforceBulkIterator takes the preconfigured queries and executes those queries in order
